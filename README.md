...
Академические и наукометрические числа могут устаревать, потому что они записаны в JSON/HTML и не запрашиваются из внешних сервисов при каждом открытии страницы. Плановый сбор обновляет данные через GitHub Actions; при недоступности источника сохраняются прежние значения и отдельно фиксируется его состояние. Галерея дипломов содержит публичные миниатюры и ссылки на документы, уже присутствующие в репозитории; ее не следует расширять приватными удостоверениями или непубличными материалами без явного разрешения.
