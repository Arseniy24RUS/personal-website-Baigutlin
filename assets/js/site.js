(function(){
  ...
  async function renderPublications(){
    ...
    container.innerHTML = rows.map(row => {
      ...
      const badges = sources.map(source => `<span class="badge">${escape(source)}</span>`).join('') + (Number.isFinite(citations) && citations > 0 ? `<span class="badge light">${language === 'en' ? 'RSCI citations' : 'РИНЦ цит.'}: ${citations}</span>` : '');
